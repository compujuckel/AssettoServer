...
{
    extension(IniData data)
    {
        public void Deserialize(object target, Type type, string? section = null)
        {
        ...
        public void Deserialize<T>(T target, string? section = null) where T : class, new()
        {
            data.Deserialize(target, typeof(T), section);
        }

        public T DeserializeObject<T>() where T : class, new()
        {
            T ret = new T();
